...
def load_fixed_events() -> List[Event]:
    ...
    if data:
        # [해결] image_42ac4e 에러 수정 (괄호 위치 조정)
...
paid_f_sum = sum(e.amount for e in period_fixed if e.date <= actual_today)
rem_f_sum = sum(e.amount for e in period_fixed if e.date > actual_today)

# 아구 맞추기 로직 (남는 자금 예비비 자동 할당)
...
# [해결] image_43ab37 에러 수정 (f-string 중괄호 및 점 위치 수정)
...
if view_mode == "PC 모드 (달력형)":
    ...
    for day in date_list:
        ...
        # [해결] image_43aeba 에러 수정 (sum 괄호 닫기)
        ...
        # [해결] image_440898 깨짐 방지: f-string HTML 구성 및 unsafe_allow_html 적용
