...
food_budget = st.sidebar.number_input("🍔 식비 예산", min_value=0, value=650000, step=10000)
em_budget = st.sidebar.number_input("🚨 예비비 예산", min_value=0, value=200000, step=10000)
...
with st.sidebar.expander("💰 수입/고정비 수정"):
    ...
    st.markdown("---")
    fixed_df = pd.DataFrame([{"date": e.date.isoformat(), "item": e.item, "amount": e.amount} for e in st.session_state.fixed_events])
...
total_rem = cur_inc - f_total - spent_total
...
st.markdown("#### 🎯 예산 차감 현황")
b1, b2 = st.columns(2)
b1.metric("🍔 식비 잔액", f"{rem_food:,.0f}원", delta=f"지출: {spent_food:,.0f}", delta_color="inverse")
b2.metric("🚨 예비비 잔액", f"{rem_em:,.0f}원", delta=f"지출: {spent_em:,.0f}", delta_color="inverse")
