...
st.markdown("""
    <style>
        ...
        .today-card { border: 2.5px solid #ccff00 !important; }
        /* 메트릭 글자 크기 축소 (기존보다 작게) */
...
def load_fixed_events() -> List[Event]:
    ...
    if data:
        # [해결] image_42ac4e.png 에러의 원인인 괄호 오타 수정
...
other_budget_total = st.sidebar.number_input("➕ 월 기타 예산", min_value=0, value=50000)
...
monthly_savings = st.sidebar.number_input("이번 달 파킹(저금)", min_value=0, value=cash_month_data["savings"])
withdrawal = st.sidebar.number_input("비상금 인출", min_value=0, value=cash_month_data["withdrawal"])
...
if st.sidebar.button("💰 자산 현황 업데이트"):
    ...
    diff = (monthly_savings - old_data["savings"]) - (withdrawal - old_data["withdrawal"])
...
# [해결] image_43ab37.png 에러의 원인인 f-string 오타 수정
...
if surplus > 0: st.info(f"💡 남는 자금 {surplus:,.0f}원을 예비비에 자동 할당하여 아구를 맞췄습니다.")
...
b_cols[5].metric("✅ 정산 총합", f"{total_balance:,.0f}원")
...
if view_mode == "PC 모드 (기존 달력)":
    ...
    for day in date_list:
        ...
        # [해결] image_43aeba.png 에러의 원인인 sum 괄호 누락 수정
        f_sum = sum(e.amount for e in day_f); s_sum = sum(r['amount'] for r in day_s)
        
        html = f"""<div class="mobile-card {'today-card' if is_today else ''}" style="border-left-color: {'#e74c3c' if day_f else '#3498db'};">
            <div style="display: flex; justify-content: space-between;"><b>{day.strftime('%m/%d')} ({['월','화','수','목','금','토','일'][day.weekday()]})</b>
            <span style="font-size:12px; color:#666;">합계: {f_sum + s_sum:,.0f}원</span></div>"""
        if withdrawal > 0 and withdrawal_date == day:
            html += f"<div style='background:#ff4b4b; color:white; padding:4px; border-radius:6px; font-size:11px; margin-top:5px;'>💸 비상금 인출: {withdrawal:,.0f}원</div>"
        if day_f:
            html += f"<div style='color:#e74c3c; font-size:12px; margin-top:5px; font-weight:bold;'>📌 고정비</div>"
            for e in day_f: html += f"<div style='font-size:11px; margin-left:10px;'>· {e.item} ({e.amount:,.0f})</div>"
        if day_s:
            html += f"<div style='color:#3498db; font-size:12px; margin-top:5px; font-weight:bold;'>🛒 변동비</div>"
