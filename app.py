...
def build_fixed_events(year=YEAR, start_month=START_MONTH, end_month=END_MONTH) -> List[Event]:
    ...
    for m in range(start_month, end_month + 1):
        ...
        try:
            ...
            month_end_date = dt.date(year, m, ld_val)
            events.append(Event(next_monday_if_weekend(month_end_date), "주거/공과금", "관리비", 110000))
        except Exception:
            pass

    return [
        e for e in events
        if not (e.date.year == YEAR and e.date.month == 2 and e.date < feb_cutoff)
    ]
...
# -----------------------------
# 내일부터 사용 기준
actual_today = dt.date.today() + dt.timedelta(days=1)
...
st.sidebar.info(f"✨ 기준일(내일부터): {actual_today.strftime('%Y-%m-%d')}")
...
if isinstance(saved_val, dict) and "start" in saved_val:
    saved_dates = saved_val
else:
    saved_dates = {"start": f"{YEAR}-{selected_month:02d}-01", "end": f"{YEAR}-{selected_month:02d}-24"}

budget_start = st.sidebar.date_input("📅 예산 시작일", value=dt.date.fromisoformat(saved_dates["start"]))
budget_end = st.sidebar.date_input("🏁 예산 종료일", value=dt.date.fromisoformat(saved_dates["end"]))
...
with st.sidebar.expander("📝 수입 및 고정비 항목 수정"):
    income_df = pd.DataFrame([
        {"월": k, "수입(원)": v}
        for k, v in st.session_state.income_data.items()
    ])
...
# 내일부터만 보이게 시작점 조정
visible_start = max(budget_start, actual_today)
...
period_fixed_events = [
    e for e in st.session_state.fixed_events
    if visible_start <= e.date <= budget_end
]
...
v_period_df = df[
    (df["date"].dt.date >= visible_start) &
    (df["date"].dt.date <= budget_end)
]
...
start_weekday = visible_start.weekday()
...
weekly_food_base = food_budget_total / len(weeks) if len(weeks) > 0 else 0
weekly_balances = {}
...
for week_row in weeks:
    ...
    w_food_spent = int(
        v_period_df[
            (v_period_df["category"] == "식비") &
            (v_period_df["date"].dt.date >= ws) &
            (v_period_df["date"].dt.date <= we)
        ]["amount"].sum()
    )
...
total_remaining_budget = (
    total_food_surplus_deficit +
    (hh_budget_total - used_hh) +
    (tr_budget_total - used_tr) +
    (other_budget_total - used_other) +
    (em_budget_total - used_em)
)

# ---------- Main (대시보드) ----------
st.title(f"📅 {selected_month}월 예산 달력")

st.markdown(f"### 📊 자금 흐름 현황 (기간: {visible_start.strftime('%m/%d')} ~ {budget_end.strftime('%m/%d')})")
m1, m2, m3, m4, m5 = st.columns(5)
m1.metric("월 수입", f"{cur_inc:,.0f}원")
m2.metric(
    "💰 총 현금 자산 (파킹)",
    f"{st.session_state.cash_data['total_balance']:,.0f}원",
    delta=f"- {withdrawal:,.0f} (비상금)" if withdrawal > 0 else None,
    delta_color="normal"
)
...
st.markdown(
    f"### 🚩 자금 설계 진단: <span style='color:{diag_color};'>{'적정 (여유로움)' if adequacy >= 0 else '예산 초과 (조정 필요)'}</span>",
    unsafe_allow_html=True
)
...
result_text = (
    f"💡 결과: 현재 계획상 **{abs(adequacy):,.0f}원**이 부족합니다."
    if adequacy < 0
    else f"💡 결과: 남는 자금 **{surplus:,.0f}원**을 예비비에 자동 할당하여 아구를 맞췄습니다."
)
st.info(result_text)
...
with st.expander("➕ 지출 추가하기", expanded=True):
    ...
    with c1:
        d = st.date_input("날짜", value=visible_start)
    with c2:
        cat = st.selectbox("분류", ["식비", "생활용품", "교통/차량", "기타", "예비비"])
    with c3:
        memo = st.text_input("내용")
    with c4:
        amt = st.number_input("금액(원)", min_value=0, step=1000)
...
# 공통 데이터 매핑
f_date_map: Dict[dt.date, List[Event]] = {}
s_date_map: Dict[dt.date, List[Tuple[str, int, str]]] = {}
...
if view_mode == "PC 모드 (달력형)":
    ...
    st.caption(f"📍 기준일은 {actual_today.strftime('%m월 %d일')}이며, 연두색 테두리로 표시됩니다.")

    header_cols = st.columns(7)
    for idx, name in enumerate(["월", "화", "수", "목", "금", "토", "일"]):
        header_cols[idx].markdown(
            f"<div style='text-align:center; background:#eee; padding:5px; border-radius:5px;'><b>{name}</b></div>",
            unsafe_allow_html=True
        )
    ...
    for week in weeks:
        ...
        for idx, current_date in enumerate(week):
            if current_date is None:
                cols[idx].markdown(
                    "<div style='height:280px; background:#f9f9f9; border:1px solid #eee; border-radius:8px;'></div>",
                    unsafe_allow_html=True
                )
                continue
            ...
            cell_html = [
                f"<div class='cal-content'><div style='font-weight:bold; color:{'#2ecc71' if is_today else '#333'};'>{current_date.month}/{current_date.day} {'(기준일)' if is_today else ''}</div>"
            ]

            w_info = st.session_state.cash_data["monthly"].get(month_key, {})
            has_withdrawal = (
                w_info.get("withdrawal", 0) > 0 and
                dt.date.fromisoformat(w_info.get("withdrawal_date")) == current_date
            )
            if has_withdrawal:
                cell_html.append(
                    f"<div style='background-color:#ff4b4b; color:white; padding:5px; border-radius:5px; font-weight:bold; text-align:center; font-size:13px; margin-bottom:8px;'>💸 비상금 인출: {w_info['withdrawal']:,.0f}원</div>"
                )
            ...
            if day_fixed:
                ...
                for e in day_fixed:
                    cell_html.append(f"<div style='color:#c0392b; font-size:11px;'>· {e.item} ({e.amount:,.0f})</div>")
            ...
            if day_spent:
                ...
                for c, a, m in day_spent:
                    cell_html.append(f"<div style='color:#2980b9; font-size:12px;'>· {m if m else c} ({a:,.0f})</div>")
            ...
            cols[idx].markdown(
                f"<div style='height:280px; border:{border_style}; padding:8px; background:{bg_style}; border-radius:{rad_style}; overflow-y:auto;'>{''.join(cell_html)}</div>",
                unsafe_allow_html=True
            )

else:
    st.subheader("📱 일자별 지출 내역 (모바일 리스트)")
    st.caption("내용이 있는 날짜와 기준일만 표시됩니다.")
    ...
    for current_date in date_list:
        ...
        today_badge = "<span style='color:#2ecc71; font-weight:bold; font-size:14px;'>[기준일]</span>" if is_today else ""
        html_lines.append("<div style='display:flex; justify-content:space-between; align-items:center; border-bottom: 1px solid #eee; padding-bottom: 8px; margin-bottom: 8px;'>")
        ...
        if has_withdrawal:
            html_lines.append(
                f"<div style='background-color:#ff4b4b; color:white; padding:6px; border-radius:5px; font-weight:bold; text-align:center; font-size:13px; margin-bottom:8px;'>💸 비상금 인출: {w_info['withdrawal']:,.0f}원</div>"
            )

        if current_date.weekday() == 6 or current_date == budget_end:
            weeks_grid = [([None] * visible_start.weekday() + date_list)[i:i + 7] for i in range(0, len([None] * visible_start.weekday() + date_list), 7)]
            weekly_food_base_mobile = food_budget_total / len(weeks_grid) if len(weeks_grid) > 0 else 0
            w_bal = None
            ...
            for week_row in weeks_grid:
                ...
                week_end_ref = week_row[6] if (len(week_row) > 6 and week_row[6]) else valid[-1]
                if week_end_ref == current_date:
                    ws, we = valid[0], valid[-1]
                    w_food_spent = int(
                        v_period_df[
                            (v_period_df["category"] == "식비") &
                    ...
                    w_food_bal = weekly_food_base_mobile - w_food_spent
                    cum_spent_df = v_period_df[v_period_df["date"].dt.date <= we]
                    w_bal = {
                        "food": w_food_bal,
                        "hh": hh_budget_total - int(cum_spent_df[cum_spent_df["category"] == "생활용품"]["amount"].sum()),
...
if not v_period_df.empty:
    ...
    st.info("현재 표시 기간에는 지출 내역이 없습니다.")